...
import { useAddGeneratorLogMutation } from "../redux/api/routesApi";
...
const AddGeneratorLog = () => {
  const [form] = Form.useForm();
  const navigate = useNavigate();
  const [loading, setLoading] = useState(false);
  const [addGeneratorLog] = useAddGeneratorLogMutation();
  ...
  const handleSubmit = async (values) => {
    ...
    try {
      ...
      const res = await addGeneratorLog(payload).unwrap();
      message.success(res?.message || "Generator log added successfully");
      form.resetFields();
      ...
      <div className="max-w-site mx-auto px-4 lg:px-6 2xl:px-8 py-10">
        ...
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-1">
            ...
            <h1 className="text-3xl font-bold text-[#1A1A1A]">Log Generator Hours</h1>
          </div>
          <p className="text-[#5A5A5A] text-sm ml-4 pl-3">
            Total hours are automatically calculated from all entries and synced to your RV profile.
          </p>
        </div>
        ...
        <div className="max-w-2xl">
          <Form form={form} onFinish={handleSubmit} layout="vertical">
            ...
            <div className="flex gap-3">
              ...
              >
                {loading ? <><Spin size="small" /><span>Saving…</span></> : "Save Generator Log"}
              </button>
            </div>

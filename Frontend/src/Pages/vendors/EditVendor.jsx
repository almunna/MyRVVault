  ...
  useEffect(() => {
    const fetchVendor = async () => {
      try {
        ...
        if (data.success) form.setFieldsValue(data.data);
      } catch {
  ...
  const handleSubmit = async (values) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        message.error(data.message || "Failed to update vendor");
      }
    } catch {
  ...
  return (
    <div className="container m-auto py-8 px-3 lg:px-0 max-w-2xl">
      <div className="flex items-center gap-4 mb-6">
        ...
        <h1 className="text-2xl font-semibold text-[#F9B038]">Edit Vendor</h1>
      </div>

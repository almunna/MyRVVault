...
import { useAddComponentMutation } from "../redux/api/routesApi";
...
const ComponentAdd = () => {
  ...
  const handleSubmit = async (values) => {
    ...
    try {
      ...
      const res = await addComponent({ urlPath: cfg.urlPath, data: formData }).unwrap();
      message.success(res?.message || "Component saved");
        ...
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-1">
            ...
            <h1 className="text-3xl font-bold text-[#1A1A1A]">
              <span className="mr-2">{cfg.icon}</span>Add {cfg.label}
            </h1>
          </div>
          <p className="text-[#5A5A5A] text-sm ml-4 pl-3">Track this component with full health and warranty monitoring</p>
        </div>
          ...
          <Form form={form} onFinish={handleSubmit} layout="vertical">
            ...
            <div className="flex gap-3">
              ...
              <button type="submit" disabled={loading}
                ...
                {loading ? <><Spin size="small" /><span>Saving…</span></> : `Save ${cfg.label}`}
              </button>
            </div>
          </Form>
        </div>
